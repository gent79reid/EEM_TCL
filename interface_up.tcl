::cisco::eem::event_register_syslog occurs 1 pattern $_syslog_pattern_ipsla_up maxrun 240
#------------------------------------------------------------------
...
if { ! [info exists _syslog_pattern_ipsla_up] } { 

	error "Environment variable _syslog_pattern_ipsla_up is NOT set"
	exit 1
...
if [catch {cli_exec $cli1(fd) "no shut"} result] {
	    ...
	    puts "The IP SLA status become up, the interface $_config_interface has been no shutdown via EEM"
	}
